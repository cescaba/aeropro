...
trait VC_Onboarding_Wizard_Shortcodes {
  ...
  public function shortcode_check_email(): string {
    ...
    $sign_in_url = function_exists('pmpro_login_url') ? pmpro_login_url() : wp_login_url();
    $reset_password_url = function_exists('pmpro_login_url')
      ? add_query_arg('action', urlencode('reset_pass'), pmpro_login_url())
      : wp_lostpassword_url();
    $notices_html = '';
    if (isset($_GET['verify']) && $_GET['verify'] === 'expired') {
  ...
  public function shortcode_custom_login($atts = [], $content = null): string {
    ...
    $login_form_defaults_filter = static function ($defaults) {
      $defaults['label_username'] = 'Email address';
      return $defaults;
    };
    $login_nav_filter = static function ($links, $pmpro_form) {
      if ($pmpro_form === 'login' && function_exists('pmpro_login_url')) {
        $links['lost_password'] = sprintf(
          '<a href="%s">%s</a>',
          esc_url(add_query_arg('action', urlencode('reset_pass'), pmpro_login_url())),
          esc_html__('Forgot your password?', 'vc-onboarding-wizard')
        );
      }

      return $links;
    };
    add_filter('login_form_defaults', $login_form_defaults_filter);
    add_filter('pmpro_login_forms_handler_nav', $login_nav_filter, 10, 2);

    try {
      $pmpro_login_html = do_shortcode('[pmpro_login]');
    } finally {
      remove_filter('login_form_defaults', $login_form_defaults_filter);
      remove_filter('pmpro_login_forms_handler_nav', $login_nav_filter, 10);
    }
    ...
    $html = $this->render_template('templates/login/custom-login.php', [
      ...
      'pmpro_login_html' => $pmpro_login_html,
    ]);
  ...
  private function render_dashboard_profile_view(int $user_id): string {
    ...
    // Profile render data: usa nombre completo si existe y cae a display/user_login como respaldo.
    ...
    // Profile template context: centraliza valores, estado de email, avatar y datos del formulario.
    return $this->render_template('templates/dashboard/profile.php', [
      ...
      'country_code' => $this->get_user_meta_string($user_id, self::META_COUNTRY_CODE),
      'state_code' => $this->get_user_meta_string($user_id, self::META_STATE_CODE),
  ...
  private function inline_css(): string {
    return '<style>
      .vc-onboard{--vc-onboarding-radius:8px;max-width:520px;margin:40px auto;padding:24px;border:1px solid #e5e5e5;border-radius:12px;background:#fff}
      .vc-onboard h2{margin:0 0 10px}
      .vc-onboard input,.vc-onboard select{width:100%;padding:10px;border:1px solid #ccc;border-radius:var(--vc-onboarding-radius)}
