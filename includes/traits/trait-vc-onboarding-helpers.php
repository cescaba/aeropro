...
trait VC_Onboarding_Wizard_Helpers {
  ...
  private function get_profile_notice_from_query(): string {
    ...
    // Profile notices: mapea estados de query string a mensajes reutilizados por Profile y Privacy.
    if ($status === 'saved') {
    ...
    if ($status === 'location_invalid') {
      return $this->render_notice('Please select a valid country and state.', 'error');
    }
